...
export const isEth = (token?: Address) => !token || token === '0x0000000000000000000000000000000000000000';

export function fmtAmount(wei: bigint | null | undefined, token?: Address): string {
  if (wei == null) return '—';
  return isEth(token)
...
export const net = (wei: bigint): bigint => wei - (wei * FEE_BPS) / 10000n;
export const fee = (wei: bigint): bigint => (wei * FEE_BPS) / 10000n;

export function countdown(submittedAt: number, timelockSecs: number, now = Date.now()) {
