...
type NodeKey = 'P' | 'S' | 'A' | 'AR' | 'D' | 'R';
type EdgeKey = 'e1' | 'e2' | 'e3' | 'e4' | 'e5';
const C: Record<NodeKey, string> = { P: T.sub, S: T.amber, A: T.green, AR: T.auto, D: T.red, R: T.violet };
const NODES_ON: Record<MilestoneState, NodeKey[]> = {
  PENDING: ['P'], SUBMITTED: ['P', 'S'], APPROVED: ['P', 'S', 'A'],
  DISPUTED: ['P', 'S', 'D'], RESOLVED: ['P', 'S', 'D', 'R'], AUTO_RELEASED: ['P', 'S', 'AR'], CANCELLED: [],
};
const EDGES_ON: Record<MilestoneState, EdgeKey[]> = {
  PENDING: [], SUBMITTED: ['e1'], APPROVED: ['e1', 'e2'],
  DISPUTED: ['e1', 'e4'], RESOLVED: ['e1', 'e4', 'e5'], AUTO_RELEASED: ['e1', 'e3'], CANCELLED: [],
};
const EDGE_COLOR: Record<EdgeKey, string> = { e1: C.S, e2: C.A, e3: C.AR, e4: C.D, e5: C.R };
const COORDS: Record<EdgeKey, [number, number, number, number]> = { e1: [30, 30, 190, 30], e2: [190, 30, 350, 30], e3: [190, 30, 330, 8], e4: [190, 30, 250, 62], e5: [250, 62, 390, 62] };
const RECENT: Partial<Record<MilestoneState, EdgeKey>> = { SUBMITTED: 'e1', APPROVED: 'e2', DISPUTED: 'e4', RESOLVED: 'e5', AUTO_RELEASED: 'e3' };

interface StateRailProps { state: MilestoneState; countdownText?: string | null; countdownColor?: string }

export default function StateRail({ state, countdownText, countdownColor }: StateRailProps) {
  const on = NODES_ON[state];
  const eOn = EDGES_ON[state];
  const recent = RECENT[state];
  const edge = (k: EdgeKey) => ({ stroke: eOn.includes(k) ? EDGE_COLOR[k] : T.line, strokeDasharray: eOn.includes(k) ? '0' : '3 4' });
  const node = (k: NodeKey) => ({ fill: on.includes(k) ? C[k] : T.bg, stroke: on.includes(k) ? C[k] : T.dim });
  const lbl = (k: NodeKey) => (on.includes(k) ? C[k] : T.cancel);
  const txt: CSSProperties = { fontFamily: mono, fontSize: '8.5px', letterSpacing: '0.5px' };
  return (
