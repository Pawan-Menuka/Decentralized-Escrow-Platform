...
**Status: DONE** (codegen + build verified locally; deploy to Subgraph Studio is a `[HUMAN]` follow-up). Hand-wrote `subgraph/` (own `package.json`/`node_modules`, isolated from the Hardhat toolchain): `subgraph.yaml` (dataSource `FreelanceEscrow`, network `sepolia`, address `0x85DBE339432cd7960FADFef78e2E6981025bD4BA`, startBlock `11287638`, `specVersion: 1.0.0`, mapping `apiVersion: 0.0.7`) with an `eventHandlers` entry for all 15 events; `schema.graphql` with `Job`, `Milestone`, `Activity`, `Withdrawal`, `Dispute` entities (all `@entity(immutable: false)` since they're mutated post-creation — required by graph-cli 0.98); `src/mapping.ts` with one handler per event. Milestone amounts aren't in `JobCreated` (only the job total is), so `handleJobCreated` binds to the contract (`FreelanceEscrowContract.bind(event.address)`) and calls `try_getMilestones(jobId)` to read each milestone's `amount` at creation time, falling back to zero-amount placeholders if the call reverts. Resolved versions: `@graphprotocol/graph-cli@0.98.1`, `@graphprotocol/graph-ts@0.38.2`. `npm run codegen` and `npm run build` both succeed (`Build completed: build\subgraph.yaml`, wasm at `subgraph/build/FreelanceEscrow/FreelanceEscrow.wasm`). `npx hardhat test` still 137 passing (no Solidity touched). Deviation: nullable `Int` fields (`resolvedFreelancerBps`, `freelancerBps`) can't actually be set to `null` in AssemblyScript (graph-ts generates a non-nullable `i32` setter for `Int` even when the GraphQL field is nullable) — left unset (defaults to `0`) until a resolution event sets the real value, rather than fighting the codegen.
...
**Done (read path):** `frontend/src/lib/graph.js` — a typed GraphQL client over `VITE_SUBGRAPH_URL` exposing `hasSubgraph()`, `fetchJobsFor(address)` (client OR freelancer OR arbitrator, deduped, newest first), `fetchActivities(jobId)` (the journal feed) and `fetchDisputes(jobId)` (incl. each side's `evidenceCid`). `useMyJobs` now queries the subgraph via TanStack Query when `VITE_SUBGRAPH_URL` is set and **falls back to the original RPC multicall** when it isn't *or if the query errors* — both paths return the identical job shape, and it reports which one it used via `source`. This removes the designer's `TODO: swap for a subgraph / indexer query`. Verified: frontend builds; with no `VITE_SUBGRAPH_URL` the RPC path still renders the live job identically (no regression).
